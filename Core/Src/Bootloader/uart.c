...
uart_status uart_receive(uint8_t *data, uint16_t length)
{
  ...
  if (HAL_OK == HAL_UART_Receive(&BOOT_UART, data, length, UART_TIMEOUT))
  {
    status = UART_OK;
  }
...
uart_status uart_transmit_str(uint8_t *data)
{
  ...
  if (HAL_OK == HAL_UART_Transmit(&BOOT_UART, data, length, UART_TIMEOUT))
  {
    status = UART_OK;
  }
...
uart_status uart_transmit_ch(uint8_t data)
{
  ...
  if (HAL_UART_STATE_TIMEOUT == HAL_UART_GetState(&BOOT_UART))
  {
    HAL_UART_Abort(&BOOT_UART);
  }

  if (HAL_OK == HAL_UART_Transmit(&BOOT_UART, &data, 1u, UART_TIMEOUT))
  {
    status = UART_OK;
  }
  return status;
